...
- `/scan-jobs`: follow [scan-jobs](workflows/scan-jobs.md) with [job-scanning rules](rules/job-scanning.md) and [job-scan schema](schemas/job-scan.md); perform bounded, on-demand public vacancy scanning through an available read-only executor. When this package's local Python executor is available, prefer the one-shot [career scan entrypoint](scripts/career_scan.py). When an MCP host is available, use the read-only [Career MCP server](mcp/career-mcp-http-server.mjs) tool `career.scan_and_review`: its public ChatGPT surface always uses the a caller-provided source preset, and `poolMode` only changes the post-discovery Candidate Pool view. This public package intentionally includes no personal preset.
